function [b_model,exp_var] = eeg_computeAAHC(eeg,n_mod,ProgBar, IgnorePolarity)
    ...
    while (nClusters) > min(n_mod)
        ...
        for f = 1:numel(ClusterToUpdate)  % Some cluster center need an update
            ...
            if (IgnorePolarity == true);        
                ...
                Cluster(ClusterToUpdate(f),:) = NormDimL2(pc1',2) / sqrt(n_chan);
            else
                Cluster(ClusterToUpdate(f),:) = NormDimL2(mean(eeg(ClusterMembers,:),1),2) / sqrt(n_chan);
            end
