...
StudyFolder = 'E:\study\Normal_s\pre\features_2';       %for the input files path of pre results 
StudyFolder_2 = 'E:\study\Normal_s\post\features_2';    %for the input files path of post results 

type='REC';                                             %for the type of RE
% FilesALL = dir([StudyFolder,'\*','tp1','*']);
FilesALL = dir(StudyFolder);
FilesALL=FilesALL(3:end);
...
FilesALL = dir(StudyFolder_2);
FilesALL=FilesALL(3:end);
for issub = 1:length(FilesALL) 
    Subfolder = [StudyFolder_2 '\' FilesALL(issub).name];
    namelist=dir([Subfolder,'\*',type,'*.','mat']);
