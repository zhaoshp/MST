...
%
function [MSClass,gfp, fit] = AssignMStates(eegdata, Maps, params, IgnorePolarity, chanlocs)

    if nargin > 4
        eegdata = pop_interp(eegdata,chanlocs,'spherical');
    end
    ...
    nSegments = size(eegdata.data,3);
    MSClass = zeros(size(eegdata.data,2),nSegments);

    gfp = std(eegdata.data,1,1);
    ...
    if params.PeakFit == 1
        ...
        for s = 1:nSegments
            ...
            Cov = NormDimL2(Maps,1) * squeeze(eegdata.data(:,IsIn,s)) / size(Maps,2);
            ...
            AllMVar = AllMVar +  squeeze(sum(std(eegdata.data(:,IsIn,s),1,1),2));
            for c = 1:nClasses
                PeakAssignment(c,GFPPClass == c) = 1;
                Fit(c,:) = interp1(IsIn,PeakAssignment(c,:),1:size(eegdata.data,2),'linear');
            end
            Fit(isnan(Fit)) = 0;
        ...
        MSClass = zeros(size(eegdata.data,2),nSegments);
        for s = 1:nSegments
            [Winner,ExpVar] = SmoothLabels(eegdata.data(:,:,s),Maps,params, eegdata.srate,IgnorePolarity);
            % Kill microstates truncated by boundaries
            if params.BControl == true
                for b = 1:numel(BoundaryPoint)
                    if BoundaryEpoch(b) == s
