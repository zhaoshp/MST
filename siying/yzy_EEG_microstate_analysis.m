...
groups = {'preprocessed','post'};
conditions = {'EC','EO'};
group_index=1;% note group 2
...
rootdir = 'E:\zhaoshp\EEG';
outdir='E:\zhaoshp\siying_MST_test';%自动生成features2文件夹存放结果
ConDir = [rootdir,filesep,groups{group_index}];%数据存放路径，某一组别
...
SavePath = [outdir,filesep,'features2',filesep,'result_',groups{group_index}];mkdir(SavePath);
ConIndex = [];
...
DirCon = dir(fullfile(ConDir,['*',conditions{cond_index},'*.mat']));  %%%% find all the set file in your folder
